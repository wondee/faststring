...
=======
			\IF {$r$ nicht markiert}
				\STATE $def \gets$ \texttt{$defUse$.getDef($r$)}
				\IF {$def$ ist eine \texttt{PhiNode}} 
					\STATE \texttt{$phis$.add($def$)}
				\ELSE
					\IF {$def$ ist vom Typ \texttt{Labelable}}
						\STATE \texttt{processInstruction($def$)}
					\ENDIF
				\ENDIF
				\FOR {$use$ \bf{in} \texttt{$defUse$.getUses($r$)}}
					\IF {$use$ ist eine \texttt{PhiInstruction}} 
						\STATE \texttt{$phis$.add($use$)}
					\ELSE
						\IF {$use$ ist vom Typ \texttt{Labelable}}
